...
final class LunarEventCalculator
{
    ...
    public function conjunctionVisualGeometry(DateTimeImmutable $instant,string $targetId,float $lat,float $lon):?array
    {
        $target=null;foreach(ConjunctionCatalog::targets() as $candidate)if($candidate->id===$targetId){$target=$candidate;break;}if($target===null)return null;
        $instant=$instant->setTimezone(new DateTimeZone('UTC'));$moon=$this->coordinates->calculate($instant,$lat,$lon);$other=$target->kind==='planet'?$this->targets->target($target->id,$instant):$this->targets->fixed($target,$instant);$horizontal=$this->horizontal($other,$instant,$lat,$lon);$relative=$this->relative($moon->altitudeDegrees,$moon->azimuthDegrees,$horizontal['altitude_degrees'],$horizontal['azimuth_degrees']);
        return['moon_altitude_degrees'=>$moon->altitudeDegrees,'moon_azimuth_degrees'=>$moon->azimuthDegrees,'target_altitude_degrees'=>$horizontal['altitude_degrees'],'target_azimuth_degrees'=>$horizontal['azimuth_degrees'],'relative_x_degrees'=>$relative['x_degrees'],'relative_y_degrees'=>$relative['y_degrees'],'separation_degrees'=>$relative['separation_degrees']];
    }
    ...
    private function altitude(EquatorialCoordinates $p,DateTimeImmutable $date,float $lat,float $lon):float{return$this->horizontal($p,$date,$lat,$lon)['altitude_degrees'];}
    /** @return array{altitude_degrees:float,azimuth_degrees:float} */private function horizontal(EquatorialCoordinates $p,DateTimeImmutable $date,float $lat,float $lon):array{$jd=2440587.5+(float)$date->format('U.u')/86400;$t=($jd-2451545)/36525;$sid=$this->signed(280.46061837+360.98564736629*($jd-2451545)+.000387933*$t*$t-$t**3/38710000+$lon);$h=deg2rad($this->signed($sid-$p->rightAscensionDegrees));$phi=deg2rad($lat);$dec=deg2rad($p->declinationDegrees);$alt=asin(sin($phi)*sin($dec)+cos($phi)*cos($dec)*cos($h));$az=atan2(sin($h),cos($h)*sin($phi)-tan($dec)*cos($phi));return['altitude_degrees'=>rad2deg($alt),'azimuth_degrees'=>fmod(rad2deg($az)+540.0,360.0)];}
    /** @return array{separation_degrees:float,x_degrees:float,y_degrees:float} */private function relative(float $moonAltitude,float $moonAzimuth,float $targetAltitude,float $targetAzimuth):array{$ma=deg2rad($moonAltitude);$mz=deg2rad($moonAzimuth);$ta=deg2rad($targetAltitude);$tz=deg2rad($targetAzimuth);$moon=[cos($ma)*sin($mz),cos($ma)*cos($mz),sin($ma)];$target=[cos($ta)*sin($tz),cos($ta)*cos($tz),sin($ta)];$east=[cos($mz),-sin($mz),0.0];$up=[-sin($ma)*sin($mz),-sin($ma)*cos($mz),cos($ma)];$dot=static fn(array $a,array $b):float=>$a[0]*$b[0]+$a[1]*$b[1]+$a[2]*$b[2];$separation=acos(max(-1.0,min(1.0,$dot($moon,$target))));$position=atan2($dot($target,$east),$dot($target,$up));return['separation_degrees'=>rad2deg($separation),'x_degrees'=>rad2deg($separation)*sin($position),'y_degrees'=>-rad2deg($separation)*cos($position)];}
    ...
    private function phaseIdentifier(string $name):string{return match($name){'luna nueva'=>'new_moon','creciente'=>'waxing_crescent','cuarto creciente'=>'first_quarter','gibosa creciente'=>'waxing_gibbous','luna llena'=>'full_moon','gibosa menguante'=>'waning_gibbous','cuarto menguante'=>'last_quarter','menguante'=>'waning_crescent',default=>$name};}
